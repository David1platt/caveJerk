...
public class Action {
	...
	public enum Commands {
		HELP("help"), ATTACK("attack"), GET("get"), RUN("run"), INVENTORY("inventory"), USE("use"), OUTFIT("outfit"),
		LOOK("look"), LOAD("load"), EQUIP("equip"), SAVE("save"), QUICKSLOTS("quick"), STATS("stats"), QUIT("quit"),
		NORTH("n"), SOUTH("s"), EAST("e"), WEST("w"), DOWN("down"), UP("up");
	...
	public void commands(CoordinateManager currLoc, Driver game, int levelNum, Inventory stuff, Player playerOne,
			Map level, String noGo)
			throws IOException, StringIndexOutOfBoundsException, IndexOutOfBoundsException, InterruptedException {
		...
		while (!doStuff.keyWord.equals("quit")) {
			...
			if ((level.getRoom(currLoc).getMonsters() != null) && (doStuff.keyWord.equals(Commands.GET.word)
					|| doStuff.keyWord.contains(Commands.EQUIP.word) || doStuff.keyWord.contains(Commands.USE.word)))
				battle.enemyFreeAttack(playerOne, level.getRoom(currLoc).getMonsters());
			if ((doStuff.keyWord.equals(Commands.NORTH.word)) || (doStuff.keyWord.equals(Commands.SOUTH.word))
					|| (doStuff.keyWord.equals(Commands.EAST.word)) || (doStuff.keyWord.equals(Commands.WEST.word))) {
				...
				currLoc = level.getLvlStartPt(levelNum, doStuff.keyWord);
				level.getRoom(currLoc).format_PrintRm();
			} else if ((doStuff.keyWord.length() > 5)
					&& (doStuff.keyWord.substring(0, 5).equals(Commands.EQUIP.word))) {
				boolean check = plyrEquip(playerOne, doStuff.keyWord);// the command string is included to get the name
																		// of the item to be equipped
				if (check == false)
				...
				try {
					...
					System.out.println(
							"Please provide the full proper name of the item you would like to save in a quick slot.");
				}
				userIn.close();
				...
				String action = "You scramble away from the bloodthirsty beasts in a blind panic. ",
						warning = "You are cornered by these terrors from the deep! ";
				boolean run = false;
				enemies = level.getRoom(currLoc).getMonsters();
	...
	/*******
	 ** function takes Action, Map, Driver, Inventory, CoordinateManager, String,
	 * String args it checks if the player can go a direction then calls the
	 * movement function to change the position of the player on the map by one
	 ** checks if an encounter check has been run, if not then runs encounter check
	 * and saves any successful encounter check with enemies in a new MonserHorde
	 * object
	 * 
	 * @throws InterruptedException
	 ******/
	public boolean goPlaces(String doStuff, Map level, Driver game, Inventory stuff, CoordinateManager currLoc,
			String noGo, Player playerOne) throws InterruptedException {
	...
	private void healType(Items typeHeal, Player playerOne) {
		if ((typeHeal.getItemType().equals("light healing potion"))
				|| (typeHeal.getItemType().equals("medium healing potion"))
				|| (typeHeal.getItemType().equals("critical healing potion"))) {
