...
public class Combat
{
     ...
     **********/
    public int selectMonster(ArrayList<Monster> gang)
    {
        int select = 0;
     ...
     ** @return
     *********/
    private final ArrayList<Monster> battleResults(Player playerOne, ArrayList<Monster> gang)throws InterruptedException
    {
        Monster enemy; 
    	...
    	if(pFight > eFight)//player hits enemy
    	{
            ...
            {
            	System.out.println("You are too winded to land a hit that does damage. You crouch defensively, and take a deep breath.");
            	playerOne.setNumAttcks(1);
            	return gang;
            }
    ...
    private void enemyDoDamage(Monster enemy, ArrayList<Monster> gang, Player playerOne) throws InterruptedException
    {
  		int dmg = 0;
		int dmgDone = 0;
    ...
    /**********
     **checks combat conditions when fight ends, if player won or lost if enemy will take a free attack
     **@param playerOne
     **@param monster
     **@param melee
     **@return
     ...
     **********/
    public MonsterHorde fight(Player playerOne, MonsterHorde monster, Action melee)throws NullPointerException, InterruptedException
    {
    	String currCommand = melee.getCommand();
    	ArrayList<Monster> gang = monster.getHorde();
    	while(((currCommand.charAt(0) == 'a' || currCommand.equals("enemyAttck") || currCommand.toUpperCase().equals("Y"))|| currCommand.equals("1") || currCommand.equals("2") || currCommand.equals("3"))  && (gang.size() > 0 && playerOne.getHp() > 0))
        {
        	...
        	{
        		System.out.println("Do you want to keep fighting?: (Y)es/(N)o?\n You can also type 1, 2, 3 to use a quick item.");
    ...
    private Monster criticalHit(int pFight, int eFight, Items weapon, Player playerOne, Monster enemy)throws InterruptedException
    {
    	int hp = enemy.getHp();
     ...
     * *@param playerOne
     * *@param monster arrayList of monsters
     *********/
    public void enemyFreeAttack(Player playerOne, MonsterHorde monster)
    {
    	...
    	pFight = playerOne.getCombatScore() + playerOne.getAc();//player doubles AC when taking a noncombat action, in defensive mode.
    	enemy = gang.get(monsterOption);
    	System.out.println("a terrible " + enemy.getType() + " steps up to attack!");
    	eFight = enemy.getCombatScore();
    	pFight = pFight + initiative.nextInt(10);
    	eFight = eFight + initiative.nextInt(10);
    	//System.out.println("You rolled a " + pFight + " versus the enemy attack roll of " + eFight);
    	if(pFight > eFight)//player hits enemy
    	{
    	...
    	else//enemy hits player
    	{
    		...
    		dmgDone = playerOne.getHp() - dmg;//damage done to player
    		playerOne.setHp(dmgDone);   
    		System.out.println("CRUNCH! your opponent continues their assault and deals you " + dmg + " points of damage!" );
    		System.out.println("You now have " + dmgDone + " hit points.");
    		playerOne.setCombatScore();
    	} 
    }
    /********************************
     * *function chooses critical hit description 
     * * no parameters
     * *@return String
     *******************************/
    public String criticalHit(Items weapon)
    {
    	...
    	String[] descriptions = {	"You desperately charge headlong at your opponent, and blindly strike with all your might.",
    								"You shoulder your opponent into the wall and bring your " + options[0] + " around to drive it into the beasts short ribs.", 
   ...
   private ArrayList<Monster> weaponDamage(Items weapon, Monster enemy, Player playerOne, ArrayList<Monster> gang, boolean crit, int monsterOption) throws InterruptedException
   {
