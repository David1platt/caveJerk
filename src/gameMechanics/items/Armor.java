...
public class Armor extends Items {
	...
	public Monster itemAbility(Monster enemy, Player playerOne) throws InterruptedException {
		...
		case "chain mail":
			if (specialCheck < 36) {
				enemy.setNmattacks(0);
				String spec = "The " + enemy.getType()
						+ " strikes with teeth rattle force, but its weapon is caught in rings of your armor!";
				UI.ColorInterface.txtColorRm("cyan", spec);
			}
			return enemy;
		case "plate mail":
			if (specialCheck < 51) {
				enemy.setNmattacks(0);
				String spec = "The " + enemy.getType()
						+ " strikes with teeth rattle force, but cannot pierce the steel plate!";
				UI.ColorInterface.txtColorRm("cyan", spec);
			}
			return enemy;
		default:
