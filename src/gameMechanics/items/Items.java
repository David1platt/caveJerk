...
public class Items {
	...
	public List<Monster> applyDamage(Items weapon, List<Integer> eHit, List<Monster> gang) {
		for (Integer index : eHit) {
			Monster badGuy = gang.get(index);
			if (badGuy.getHp() > 0) {
				int hitPower = doDamage(badGuy, weapon);
				badGuy.setHp(hitPower);
				gang.set(index, badGuy);
				if (hitPower > (badGuy.getHp() / 3) && badGuy.getHp() > 2)
					gang.get(index).setCombatScore();
			} else {
				gang.remove(index);
			}
		}
