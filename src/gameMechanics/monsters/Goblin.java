...
public class Goblin extends Monster implements MonsterAction {
    ...
    protected int goblinSpecial(ArrayList<Monster> gang)
    {
    	int check = 0, dmg = 0;
    	if(gang.size() > 1)
    	{
			...
			for(Monster it : gang)
			{
				...
				if(check > 5)
				{
					...
					System.out.println("The " + it.getType() + " steps in and visciously slashes you from a weak point in your guard." );
				}
			}
    	}
    	else
