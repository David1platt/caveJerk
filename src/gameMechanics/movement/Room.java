...
public class Room {
	...
	public void randSetRoom(int lev, Player playerOne) {
		...
		if (lev == 3 && playerOne.getArtifactFound() == false) {
			...
			if (num > 8) {
				...
				System.out
						.println("You have found " + this.gear.getItemType() + ", take it! Now you must get back to the"
	...
	/******
	 ** this function takes no arguments creates an encounter including number and
	 * types of creatures to be printed on the page
	 **
	 *****/
	...
	public void format_PrintRm() throws InterruptedException {
		...
		/*
		 * if(this.gear != null && this.enemies != null) { item ="\nYou have found a " +
		 * this.gear.getItemType(); ColorInterface.txtColorRm("yellow", item); beasties
		 * ="\n\nYou still see the following group" + enemies.toString() +
		 * "moving in for the kill."; ColorInterface.txtColorRm("red", beasties);
